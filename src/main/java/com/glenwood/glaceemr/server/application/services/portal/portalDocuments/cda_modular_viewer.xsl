                  ...
                  <xsl:if test="n1:patient/n1:languageCommunication">
	                  <xsl:if test="n1:patient/n1:languageCommunication/n1:languageCode">
		                  <tr>
		                        <td width="20%" bgcolor="#3399ff">
		                           <span class="td_label">
		                              <xsl:text>Pref. Language</xsl:text>
		                           </span>
		                        </td>
 ...
 @mediaType='image/jpeg']">
   ...
   <xsl:template name="show-assignedEntity">
      ...
      <xsl:choose>
         ...
         <xsl:if test="$asgnEntity/n1:assignedPerson/n1:name/n1:prefix">
               <xsl:value-of select="$asgnEntity/n1:assignedPerson/n1:name/n1:prefix"/>
               <xsl:text> </xsl:text>
            </xsl:if>
			<xsl:for-each select="$asgnEntity/n1:assignedPerson/n1:name/n1:given">
				<xsl:value-of select="text()"/>&#160;
			</xsl:for-each>
           <!--   <xsl:if test="$asgnEntity/n1:assignedPerson/n1:name/n2:given">
            	<xsl:value-of select="$asgnEntity/n1:assignedPerson/n1:name/n2:given"/> 
             </xsl:if> -->
            <xsl:text> </xsl:text>
            <xsl:value-of select="$asgnEntity/n1:assignedPerson/n1:name/n1:family"/>
            <xsl:if test="$asgnEntity/n1:assignedPerson/n1:name/n1:suffix">
               <xsl:text> </xsl:text>
               <xsl:value-of select="$asgnEntity/n1:assignedPerson/n1:name/n1:suffix"/>
            </xsl:if>
      ...
      <!--       <xsl:if test="$asgnEntity/n1:representedOrganization/n1:name">
            ...
            <xsl:if test="$asgnEntity/n1:telecom[@value!='']">
               &#160;<xsl:value-of select="$asgnEntity/n1:telecom/@value"/>
            </xsl:if>
            <xsl:if test="$asgnEntity/n1:addr">
            
         	   <xsl:if test="$asgnEntity/n1:addr/n1:streetAddressLine"/>
          	     ,&#160;<xsl:value-of select="$asgnEntity/n1:addr/n1:streetAddressLine"/>
          	  </xsl:if>
          	   <xsl:if test="$asgnEntity/n1:addr/n1:city">
           	    ,&#160; <xsl:value-of select="$asgnEntity/n1:addr/n1:city"/>
          	   </xsl:if>
<!--            	  <xsl:if test="$asgnEntity/n1:addr/n1:country">
           	    &#160;, <xsl:value-of select="$asgnEntity/n1:addr/n1:country"/>
           	  </xsl:if> -->
				 <xsl:if test="$asgnEntity/n1:addr/n1:state">               
            	   ,&#160; <xsl:value-of select="$asgnEntity/n1:addr/n1:state"/>
           	  </xsl:if>
              <xsl:if test="$asgnEntity/n1:addr/n1:postalCode">
            	   &#160; <xsl:value-of select="$asgnEntity/n1:addr/n1:postalCode"/>
              </xsl:if>
